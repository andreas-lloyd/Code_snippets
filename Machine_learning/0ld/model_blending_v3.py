...
class blender(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, model_list = [{'log_reg' : LogisticRegression()}], n_folds = 3, scorer = accuracy_score, get_argmax = True, save_loc = None, seed = 100):
        ...
        '''
        assert type(model_list) == list and type(model_list[0]) == dict, 'Have not detected a list of dicts from model_list - must wrap it up!'
        
        # Set up our model list like an object to get working with gridsearch
    ...
    def fit(self, train_data, train_y):
        ...
        '''
        # Do a check on the length of train data - compare with model_list because the keys in model selection are taken from there
        ...
        # Then what we will do is assign some attributes to our class that represent our models
        ...
        for level_num, level in enumerate(self.model_list):
            ...
            for model_num, model in enumerate(level):
                ...
                for train_index, val_index in k_folds.split(X, self.train_y):
                    ...
                    getattr(self.model_list, model_key).fit(X_train, y_train)
                ...
                getattr(self.model_list, model_key).fit(X, self.train_y)
    ...
    def predict_proba(self, test_data):
        ...
        for level_num, level in enumerate(self.model_list):
            ...
            for model_num, model in enumerate(level):
                ...
                model_key = 'Level_{}_{}'.format(level_num + 1, model)
                model_probabilities = getattr(self.model_list, model_key).predict_proba(X) # Might need to review this step
